...
# --- 3. GERAÇÃO DE GRÁFICOS (EIXOS INVERTIDOS ONDE SOLICITADO) ---
def gerar_mini_grafico(tipo, gen, df_ref, medicoes):
    fig = go.Figure()
    curva = df_ref[(df_ref['genero'] == gen) & (df_ref['tipo'] == tipo)]
    
    # Determina se este gráfico deve ter os eixos invertidos (Idade no Y)
    inverter = tipo in ["imc_idade", "peso_idade", "estatura_idade"]
    
    col_ref_valor = 'z_0' # Usado apenas para lógica de eixos
    
    for col_z, color in [('z_3pos', 'red'), ('z_2pos', 'orange'), ('z_0', 'green'), ('z_2neg', 'orange'), ('z_3neg', 'red')]:
        if col_z in curva.columns:
            if inverter:
                # Eixos Invertidos: X = Valor (Z-score), Y = Idade
                fig.add_trace(go.Scatter(x=curva[col_z], y=curva['idade_meses'], line=dict(color=color, width=1, dash='dot' if col_z!='z_0' else 'solid'), mode='lines', hoverinfo='skip'))
            else:
                # Eixo Normal (Peso por Estatura): X = Altura, Y = Peso (Z-score)
                fig.add_trace(go.Scatter(x=curva['altura'], y=curva[col_z], line=dict(color=color, width=1, dash='dot' if col_z!='z_0' else 'solid'), mode='lines', hoverinfo='skip'))
    
    # Pontos das Medições
    for m in medicoes:
        val_valor = m['y_peso'] if 'peso' in tipo else (m['y_alt'] if 'estatura' in tipo else m['y_imc'])
        val_idade_ou_alt = m['x_alt'] if tipo == 'peso_altura' else m['x_idade']
        
        if inverter:
            # Ponto Invertido: X é o valor medido, Y é a idade
            fig.add_trace(go.Scatter(x=[val_valor], y=[val_idade_ou_alt], mode='markers+text', text=[f"<b>{val_valor}</b>"], textposition="top center", marker=dict(size=10, color=m['cor_base'], line=dict(width=1, color='white'))))
        else:
            # Ponto Normal: X é a altura, Y é o peso
            fig.add_trace(go.Scatter(x=[val_idade_ou_alt], y=[val_valor], mode='markers+text', text=[f"<b>{val_valor}</b>"], textposition="top center", marker=dict(size=10, color=m['cor_base'], line=dict(width=1, color='white'))))
