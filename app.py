...
if df_ref is not None and turmas:
    ...
    if modo == "Ficha Individual":
        ...
        if pd.isna(data_nasc):
            st.error("Data de Nascimento não encontrada na planilha!")
        else:
            ...
            for i, tri in enumerate(["1º Tri (Planilha)", "2º Tri", "3º Tri", "4º Tri"]):
                with cols_tri[i]:
                    st.subheader(tri)
                    # Adicionando o campo de data direto no app
                    data_af = st.date_input(f"Data Pesagem", value=datetime.now(), key=f"d_{i}_{aluno_nome}")
                    ...
                    if p > 0 and a > 0:
                        meses_calc = calcular_meses_exatos(data_nasc.date(), data_af)
            ...
            for idx, (slug, nome_g) in enumerate(params):
                with g_row[idx % 2]:
                    ...
                    # Ajuste dinâmico baseado nos meses calculados
                    if medicoes:
                        bx = medicoes[0]['meses']
        ...
        st.dataframe(df_atual[df_atual['peso_1'] > 0][['aluno', 'genero', 'nasc_data', 'peso_1', 'altura_1']], use_container_width=True)
else:
