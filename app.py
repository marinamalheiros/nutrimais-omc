...
if df_ref is not None and turmas:
    ...
    if modo == "Ficha Individual":
        ...
        for i, tri in enumerate(["1º Tri", "2º Tri", "3º Tri", "4º Tri"]):
            with cols_tri[i]:
                p = st.number_input(f"Peso (kg) - {tri}", value=float(dados_aluno['peso_1']) if i == 0 else 0.0, key=f"p{i}")
                a = st.number_input(f"Alt (cm) - {tri}", value=float(dados_aluno['altura_1']) if i == 0 else 0.0, key=f"a{i}")
                if p > 0 and a > 0:
        ...
        for idx, (slug, nome_g) in enumerate(params):
            with g_row[idx % 2]:
                ...
                fig.update_layout(title=f"<b>{nome_g}</b>", height=380, template="plotly_white", showlegend=False, margin=dict(l=10,r=10,t=40,b=10))
